                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-3">
                            <h2 className="text-lg md:text-xl font-bold">{selectedEvent.title}</h2>
                            <button 
                                onClick={closePopup}
                        ...
                        <div className="mb-3">
                            ...
                            <p className="text-sm md:text-base"><strong>Date:</strong> {moment(selectedEvent.start).format('MMMM D, YYYY')}</p>
                            <p className="text-sm md:text-base"><strong>Time:</strong> {moment(selectedEvent.start).format('h:mm A')} - {moment(selectedEvent.end).format('h:mm A')}</p>
                            {selectedEvent.resource?.description && (
                                <div className="mt-2">
                                    <p className="text-sm md:text-base"><strong>Description:</strong></p>
                                    <p className="mt-1 text-gray-700 text-sm md:text-base">{selectedEvent.resource.description}</p>
                                </div>
                            )}
                        </div>
